...
def writeDB(db_conn, db_cur, entry):
    sql = """ INSERT INTO gtx.upwork_sessions_qa (contractid, upwork_name, date_worked, dt_start, dt_end, cnt_segments, mins_upwork_billed, mins_snapshot_duration, mins_upwork_padded, cnt_mins_unreported_in_snapshots, cnt_mins_active, cnt_mins_inactive, avg_mouse, avg_keys) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"""
    values = (entry.contract, entry.name, entry.date, entry.startSess, entry.endSess, entry.cntnSegments, entry.billedMins, entry.workedMins, entry.paddedMins, entry.unreportedMins, entry.activeMins, entry.inactiveMins, entry.avgMouse, entry.avgKeybrd)
    ...
    for row in query:
        if row["state"] == "new":
            ...
            if x>1:
                ...
                print(prev.contract,prev.name, prev.startSess,prev.cntnSegments,prev.unreportedMins)
        else:
            crnt.newSegment(row)
