...
class obsTime():
    def __init__(self,row):
        ...
        self.events["timer"] = 0
        self.events["play 0"] = 0   # auto speed
    ...
    def isPlay(self):
        if self.event[:4] == "play":
            self.play = 1
        elif self.event[:5] == "agree" or self.event[:6] == "deleteObservation":
            self.play = 1
            self.speed = 0
    ...
    def endSegment(self):
        self.half = self.events["play 0.5"]/self.total
        self.one = self.events["play 1"]/self.total
        self.two = self.events["play 2"]/self.total
        self.three = self.events["play 3"]/self.total
        self.four = self.events["play 4"]/self.total
        self.five = self.events["play 5"]/self.total
        self.ttlPlay = self.events["play 0.5"] + self.events["play 1"] + self.events["play 2"] + self.events["play 3"] + self.events["play 4"] + self.events["play 5"]
...
def openDB(fyle="ace.json"):
    ...
    try:
            ...
            sql="""
                        ...
                        o.upwork_name,
                        va.fileid,
                        (ot.dt_startwalltime_utc) AS "date",
